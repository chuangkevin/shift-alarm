...
void routes() {
  ...
  server.on("/",HTTP_GET,[]{
    if(!portal){String page=String("<h1>裝置已連線</h1><p>")+(clockValid()?"時間已校正，裝置會自動同步班表。":"正在校正時間，請稍候。")+"</p><a class='primary-link' href='"+manageUrl+"'>管理班表與鬧鐘</a><h2>依照你的擺放方式設定</h2><p>調整螢幕方向，或在換環境時重新選擇無線網路。</p><a href='/display'>開啟裝置設定</a><p class='help'>同時按住「＋」與「－」十秒，也能重新配網。</p>";server.send(200,"text/html; charset=utf-8",devicePage(page));return;}
