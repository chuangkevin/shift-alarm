...
static void make_image(unsigned char image[1024]){memset(image,0x5a,sizeof(unsigned char)*1024);esp_image_header_t header={.magic=ESP_IMAGE_HEADER_MAGIC,.chip_id=ESP_CHIP_ID_ESP32S3};esp_image_segment_header_t segment={0};esp_app_desc_t desc={.magic_word=ESP_APP_DESC_MAGIC_WORD,.version="0.4.13",.project_name="xingzhi-cube-1.54tft-wifi"};memcpy(image,&header,sizeof(header));memcpy(image+sizeof(header),&segment,sizeof(segment));memcpy(image+sizeof(segment)+sizeof(header),&desc,sizeof(desc));}
static alarm_ota_manifest_t manifest_bytes(const unsigned char *image,size_t size){alarm_ota_manifest_t m={.board="xingzhi-cube-1.54tft-wifi",.version="0.4.13",.size=(uint32_t)size};digest(image,size,m.sha256);sign_manifest(&m);return m;}
...
int main(int argc,char **argv){assert(argc>=2);snprintf(marker_path,sizeof(marker_path),"%s/marker.bin",host_test_dir());const char *mode=argv[1];unsigned char image[1024];make_image(image);alarm_ota_manifest_t m=manifest(image);init_load();
 ...
 else if(!strcmp(mode,"maintenance-guard-sample")){assert(alarm_ota_load_staged()==ESP_OK);alarm_ota_handle_t h=0;assert(alarm_ota_begin(&m,(void*)1,&h)==ESP_OK);assert(alarm_ota_write(h,(void*)1,image,400)==ESP_OK);charging=false;assert(alarm_ota_maintenance()==ALARM_OTA_ERR_CHARGING_REQUIRED);status(ALARM_OTA_RECEIVING,false,false);charging=true;assert(alarm_ota_write(h,(void*)1,image+400,624)==ESP_OK);assert(alarm_ota_finish(h,(void*)1)==ESP_OK);status(ALARM_OTA_STAGED,true,false);}
