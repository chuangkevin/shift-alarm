...
- 最多保存四組 Wi-Fi。健康連線不因其他 SSID 訊號較強而主動切換；斷線後才依掃描結果自動切換。任何 API、頁面與紀錄都不得輸出 Wi-Fi 密碼或其 hash/token。
- 設定必須持久化。正常 OTA 不可擦除 NVS、Wi-Fi、Tailnet 身分、方向、關屏時間、班表或鬧鐘。
- 平時畫面不可每秒全畫面重繪。鬧鐘響時畫面閃爍；任一實體鍵停鈴。平時長按中間 BOOT 鍵 1.2 秒並放開後關屏，關屏後任一鍵只喚醒。關屏不能停止計時、網路、排程或響鈴。
- 全部使用者介面使用繁體中文。
