...
// Large backward corrections must not leave the watermark blocking future
// alarms. Keep small SNTP corrections inside the catch-up window replay-safe.
inline int64_t reconcileHandled(int64_t now, int64_t handled) {
  return now >= VALID_CLOCK && handled > now && handled - now > CATCHUP_SECONDS ? now : handled;
}
