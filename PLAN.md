...
- [x] 0.3.13 原始碼在 Tailnet 連線前略過後端輪詢；連線後由 bounded FreeRTOS worker 執行 HTTP，主迴圈維持按鍵與鬧鐘檢查並只套用仍有效的結果。所有實體 QR 統一放大為 version 8、scale 3；host 與 credential-free build 驗證完成後仍須刷入第二台實測。
...
- [ ] 將 0.3.13 刷入第二台，實測配網加入、設定網址、`/calendar` 與 `/update` 四種大型 QR 在 0／90／180／270 度均可掃描且不裁切。
