...
- 後端 0.1.5 與 GN100 Caddy 離線 fallback 已於 2026-09-15 部署；第二台已安裝 0.3.12，第一台仍為 0.3.8。GPIO38、Wi-Fi failover、實體 240×240 UI／按鍵、persisted staged image、離線安裝與 live OTA 仍待操作驗證。
...
- 韌體 0.3.13：原本 schedule GET、最多 8 秒 response read 與 heartbeat POST 都在 Arduino 主迴圈同步執行，這是實體按鍵取樣與到點檢查延遲的根因。後端輪詢改由單一 in-flight、8 KiB stack、priority 1 的獨立 FreeRTOS worker 執行；不綁 core，避免與既有 Tailnet/proxy task 配置衝突，且 blocking HTTP 會讓 FreeRTOS 排程其他 task。主迴圈只建立最多 1 KiB heartbeat 與固定長度 credential snapshot，並依 request/connectivity generation 套用最多 `MAX_JSON` 的 schedule result。Wi-Fi、Tailnet 或 OTA 狀態改變會使舊結果失效；OTA busy 時不啟動新 poll。worker 使用自己的 `HTTPClient`，不讀 mutable `String`／alarm vector，不套用 schedule、Preferences、display、alarm、OTA 或 UI 狀態；受控 credential buffer 在釋放前覆寫。queue/task 建立失敗只回報後端不可用，本機鬧鐘繼續運作。
- 韌體 0.3.13：所有實體 QR 統一為 version 8、scale 3；完整四模組 quiet zone 為 171×171 並置中於 240×240 邏輯畫面。配網右鍵可在兩個大型 QR 與手動 AP 資料間切換；密碼只出現在裝置本機配對畫面。第二台刷入與實體掃描待驗證。
- 韌體 0.3.13：OTA 下載與安裝在 backend poll in-flight 時固定回 `backend-poll-busy`，等 bounded poll 自行結束，不取消、不共用 `HTTPClient`；本機 discard 不使用 poll request/result 或 schedule worker 資源。schedule JSON 解析、512 筆與重複／時區／source／months 驗證、canonical serialization，以及 schedule NVS write+commit+exact readback 都在獨立 priority 1、12 KiB stack worker。worker 每次使用 `alarm_nvs` 的獨立 direct NVS handle；ESP-IDF NVS 內部序列化同 partition 操作，不跨 task 共用 `Preferences` handle。main 只在 persistence ack 與 schedule/backend generation 都有效時 swap candidate；過期或 result queue 失敗由 worker 路徑恢復先前 canonical，失敗會顯示明確 storage 狀態。本機網頁 POST 回 202，輪詢 ack 成功後才顯示已儲存。
- 韌體 0.3.13：schedule candidate 已寫入但無法 deliver/apply 或變 stale 時，每次 previous canonical restore 都必須 write+commit+exact readback 成功。任何 restore 不確定會 latch `scheduleStorageFault`，另寫小型 `schedule-fault-v1` marker，封鎖本機與遠端 schedule mutation，API 固定回 `schedule-storage-fault`，實體畫面顯示「班表儲存狀態不明；重開前請勿修改」。本次開機可繼續使用 fault 前已驗證的 RAM alarms；重開看見 marker 時不讀 schedule blob。marker 寫入本身失敗時只能保證本次開機 fail closed，power-cycle 後狀態仍有歧義；目前只提供 authenticated read-only `/api/schedule-storage-status` 診斷，不自動清除 marker。
