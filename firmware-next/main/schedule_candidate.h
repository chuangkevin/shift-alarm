...
struct Candidate {std::vector<Alarm> alarms;JsonDocument months,weekly_profiles;String revision,canonical,management_url,command_id;bool local=false,first_only=false;int64_t server_time=0,command_issued=0;bool stop_command=false;};
...
inline bool parse(const String &body,Candidate &out,String &error){
 ...
 if(doc.as<JsonObjectConst>().containsKey("weekly_profiles")){if(!weeklyprofiles::valid(doc["weekly_profiles"])){error="每週鬧鐘設定無效";return false;}out.weekly_profiles.set(doc["weekly_profiles"]);}else out.weekly_profiles.to<JsonObject>();
 if(out.weekly_profiles.overflowed()){error="每週鬧鐘設定記憶體不足";return false;}
 ...
 JsonDocument saved;saved["revision"]=out.revision;saved["timezone"]="Asia/Taipei";auto list=saved["alarms"].to<JsonArray>();for(const auto&a:out.alarms){auto item=list.add<JsonObject>();item["id"]=a.id;item["label"]=a.label;item["epoch"]=a.epoch;}saved["source"]=out.local?"local":"remote";saved["months"]=out.months;saved["weekly_profiles"]=out.weekly_profiles;saved["first_consecutive_only"]=out.first_only;serializeJson(saved,out.canonical);
