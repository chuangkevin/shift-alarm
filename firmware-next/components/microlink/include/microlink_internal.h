...
#define ML_EVT_NET_EXIT BIT9
#define ML_EVT_DERP_EXIT BIT10
#define ML_EVT_COORD_EXIT BIT11
#define ML_EVT_WG_EXIT BIT12
...
typedef struct {
    ...
    bool tls_initialized; /* DERP task owns lifecycle independently of socket. */
...
struct microlink_s {
    ...
    uint8_t old_node_public_key[32];
    ...
    ml_security_t security;
    cJSON *peer_map; /* Coordination task owns authoritative NodeID map. */
    bool peer_map_dirty;
...
static inline void *ml_psram_malloc(size_t size) {
    ...
    if (!ptr && size < 4096) ptr = malloc(size);
    return ptr;
}
...
static inline void *ml_psram_calloc(size_t n, size_t size) {
    ...
    if (!ptr && n * size < 4096) ptr = calloc(n, size);
    return ptr;
}
