...
bool savedScheduleRestored=true, deviceRoutesReady=false;
...
void startPortal() { if(portal)return;alarm_proxy_stop();proxyStarted=false; portal=true; WiFi.mode(WIFI_AP_STA); WiFi.softAP(apName.c_str(),apPassword.c_str());dns.start(53,"*",WiFi.softAPIP());WiFi.scanNetworks(true); Serial.println("SETUP_AP_STARTED");Serial.println(String("SETUP_AP_SSID ")+apName);Serial.println("SETUP_URL http://192.168.4.1"); }
...
</style><header><div class="brand-icon">◷</div><div class="brand"><small>每個上班日，準時提醒</small><strong>班表鬧鐘</strong></div><span class="badge">裝置本地設定</span></header><main><section class="card">)PAGE")+content+"</section></main><footer>設定保存在你的裝置 · 重新開機仍會保留</footer></html>";
}
bool localNonce();
...
esp_err_t otaDiagnostics(void*){
  ...
  NetworkClient probe;bool accepting=probe.connect(IPAddress(127,0,0,1),80,500)==1;probe.stop();
...
String tailnetLabel(alarm_tailnet_state_t state) {
  switch(state){case ALARM_TAILNET_CONNECTED:return "私人網路已連線";case ALARM_TAILNET_AUTH_REQUIRED:return "等待登入授權";case ALARM_TAILNET_EXPIRED:return "授權已到期";case ALARM_TAILNET_BLOCKED:return "存取規則尚未通過";case ALARM_TAILNET_CONNECTING:return "正在連線";default:return "尚未連線";}
}
...
void tailnetRoutes(){
  server.on("/tailnet",HTTP_GET,[]{
    String page=R"HTML(<h1>私人網路</h1><p>裝置會自己加入你的私人網路，換環境後只要設定新的無線網路即可重新連線。</p><div id="status">正在取得連線狀態…</div><p id="address"></p><p id="expiry"></p><a id="approve" class="primary-link" hidden target="_blank" rel="noopener noreferrer">前往官方頁面登入授權</a><form id="join"><button>加入／重新授權</button></form><p class="help">帳號密碼只輸入在官方登入頁，裝置不會收取密碼。授權頁需要網際網路；手機可切換到行動網路完成登入。</p><a href="/display">返回裝置設定</a><script>
const nonce='NONCE',statusEl=document.querySelector('#status'),link=document.querySelector('#approve');
async function refresh(){try{const r=await fetch('/api/tailnet',{headers:{'X-Setup-Nonce':nonce}});if(!r.ok)throw Error();const d=await r.json();statusEl.textContent=d.label;document.querySelector('#address').textContent=d.ip?'私人位址：'+d.ip:'';document.querySelector('#expiry').textContent=d.expires_at>0?'授權有效期限：'+new Date(d.expires_at*1000).toLocaleString('zh-TW'):'';link.hidden=true;if(d.auth_url){const u=new URL(d.auth_url);if(u.protocol==='https:'&&u.hostname==='login.tailscale.com'){link.href=u.href;link.hidden=false;}}}catch(e){statusEl.textContent='無法取得狀態，請確認仍連上裝置網路。'}}
document.querySelector('#join').onsubmit=async(e)=>{e.preventDefault();const r=await fetch('/api/tailnet/join',{method:'POST',headers:{'X-Setup-Nonce':nonce}});statusEl.textContent=r.ok?'正在準備授權…':await r.text();};refresh();setInterval(refresh,3000);
</script>)HTML";page.replace("NONCE",setupNonce);server.send(200,"text/html; charset=utf-8",devicePage(page));
  });
  server.on("/api/tailnet",HTTP_GET,[]{if(!localNonce())return;alarm_tailnet_status_t t={};alarm_tailnet_get_status(&t);JsonDocument d;d["label"]=tailnetLabel(t.state);d["ip"]=t.ip;d["expires_at"]=t.expires_at;d["auth_url"]=t.auth_url;d["acl_ready"]=t.acl_ready;d["peer_count"]=t.peer_count;d["peer_capacity"]=t.peer_capacity;if(t.capacity_exceeded)d["label"]="私人網路裝置數超過容量，請更新韌體";String out;serializeJson(d,out);server.sendHeader("Cache-Control","no-store");server.send(200,"application/json",out);});
  server.on("/api/tailnet/join",HTTP_POST,[]{if(!localNonce())return;if(!WiFi.isConnected()||!clockValid()){server.send(409,"text/plain; charset=utf-8","請先連上無線網路並完成校時");return;}esp_err_t e=tailnetStarted?alarm_tailnet_reauth():alarm_tailnet_start(apName.c_str());if(e==ESP_OK)tailnetStarted=true;server.send(e==ESP_OK?202:503,"text/plain; charset=utf-8",e==ESP_OK?"等待官方授權":"無法啟動私人網路，請稍後重試");});
}
...
void routes() {
  ...
  const char *headers[]={"Authorization","X-Setup-Nonce"}; server.collectHeaders(headers,2);tailnetRoutes();otaRoutes();
  ...
  server.on("/display",HTTP_GET,[]{
    ...
    page+="</select><br><button>儲存方向</button></form><p>鬧鐘響時，按任一實體按鈕即可停止。</p><h2>韌體更新</h2><p>下載已發布版本，保留舊版供失敗時回復。</p><a href='/update'>檢查裝置更新</a><h2>私人網路連線</h2><p>登入授權，讓裝置在不同環境仍能同步班表。</p><a href='/tailnet'>設定私人網路</a><h2>更換無線網路</h2><p>按下後裝置會開啟配網熱點，掃描螢幕條碼即可重新選擇網路。原設定會保留到新網路連線成功。</p><form method='post' action='/wifi/reset'><input type='hidden' name='nonce' value='"+setupNonce+"'><button>重新設定無線網路</button></form><p>無法連上此頁時，可同時按住「＋」與「－」十秒，啟動配網。</p><a href='/'>返回</a>";
...
void setup() {
  ...
  configTime(8*3600,0,"pool.ntp.org","time.google.com");routes();bootMs=millis();lastPoll=millis()-POLL_MS;draw();
...
void loop() {
  ...
  if(!portal&&WiFi.isConnected()&&!proxyStarted){if(alarm_proxy_start()==ESP_OK)proxyStarted=true;}
