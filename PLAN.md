...
- [x] 0.3.9／後端 0.1.3 原始碼完成電池與裝置可靠性功能及可在本機執行的 host 測試；未部署。

## 目前可接續工作

- [ ] 在有 ESP-IDF 5.3.2 的環境完成 0.3.9 build／size。
- [ ] 實機驗證 ADC2 channel 6 曲線、上游定義的 GPIO38 active-high 充電訊號、低電量與未知標記，以及 240×240 畫面不遮住 QR、時鐘、下次鬧鐘、配網與響鈴內容。
- [ ] 用真實中斷完成 HTTP Range 續傳與 reconnect handshake 驗收，並驗證線上入口切換 `/device-offline`；在完成前不部署。
...
- 0.3.9 的 ADC、實體 TFT 排版、真實 Range reconnect 與 live fallback 尚未在硬體／實際網路驗證。原始碼或 host 測試通過不代表已部署。
