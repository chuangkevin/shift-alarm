...
static esp_err_t load_or_generate_keys(microlink_t *ml) {
    ...
    if(err!=ESP_OK)return err; /* Never silently create an ephemeral identity. */
    const char *names[]={NVS_KEY_MACHINE_PRI,NVS_KEY_WG_PRI,NVS_KEY_DISCO_PRI};
    uint8_t *pri[]={ml->machine_private_key,ml->wg_private_key,ml->disco_private_key};
    uint8_t *pub[]={ml->machine_public_key,ml->wg_public_key,ml->disco_public_key};
    for(int i=0;i<3;i++) {
        size_t len=32;err=nvs_get_blob(nvs,names[i],pri[i],&len);
        if(err==ESP_ERR_NVS_NOT_FOUND) {
            generate_keypair(pri[i],pub[i]);err=nvs_set_blob(nvs,names[i],pri[i],32);
        } else if(err==ESP_OK&&len==32) {
            x25519_base(pub[i],pri[i],1);
        } else {if(err==ESP_OK)err=ESP_ERR_INVALID_SIZE;break;}
        if(err!=ESP_OK)break;
    }
    if(err==ESP_OK){
        size_t old_len=32;
        esp_err_t old_err=nvs_get_blob(nvs,"old_node_pub",ml->old_node_public_key,&old_len);
        if(old_err!=ESP_OK)memset(ml->old_node_public_key,0,32);
        err=nvs_commit(nvs);
    }
...
microlink_t *microlink_init(const microlink_config_t *config) {
    ...
    if(!ml_security_init(ml)){free(ml);return NULL;}
...
esp_err_t microlink_stop(microlink_t *ml) {
    ...
    /* Wake blocking sockets, then require explicit exit acknowledgements.
    ...
    if(ml->coord_task)needed|=ML_EVT_COORD_EXIT;
    if(ml->wg_mgr_task)needed|=ML_EVT_WG_EXIT;
    if(needed&&(xEventGroupWaitBits(ml->events,needed,pdFALSE,pdTRUE,pdMS_TO_TICKS(20000))&needed)!=needed)return ESP_ERR_TIMEOUT;
    ...
    if(ml->derp_tx_queue)ml_derp_disconnect(ml);
...
void microlink_destroy(microlink_t *ml) {
    ...
    if(microlink_stop(ml)!=ESP_OK)return;

    /* Deinitialize peer NVS */
    ml_security_destroy(ml);
    cJSON_Delete(ml->peer_map);
    ml_peer_nvs_deinit();

    /* Deinitialize HTTP config server */


    /* Drain heap-owned messages only after every producer has exited. */
    ml_peer_update_t *pending_peer;
    if(ml->peer_update_queue)while(xQueueReceive(ml->peer_update_queue,&pending_peer,0)==pdTRUE)free(pending_peer);
    ml_rx_packet_t pending_rx;
    QueueHandle_t rx_queues[]={ml->disco_rx_queue,ml->wg_rx_queue,ml->stun_rx_queue};
    for(unsigned i=0;i<3;i++)if(rx_queues[i])while(xQueueReceive(rx_queues[i],&pending_rx,0)==pdTRUE)free(pending_rx.data);
