...
新版韌體位於 `firmware-next/`。原始碼版本為 0.3.10，後端／映像版本仍為 0.1.3；尚未部署。實機仍為 0.3.8，後端仍為 0.1.2。0.3.10 把 OTA 分成「下載並保存」與「本機驗證後安裝」，兩步都只允許 GPIO38 當下有效且 active-high 顯示充電時執行。這是刻意 fail-closed 的充電狀態，不是可靠的 USB／VBUS 偵測；電池充滿時即使接著 USB，也可能拒絕下載或安裝。程式與 host 測試不能取代實機驗收；GPIO38、240×240 更新資訊頁、真實 flash marker、離線安裝與 live OTA 均待實機驗證。
...
- 螢幕方向可保存為 0／90／180／270 度；閒置關屏可選 1／5／15／30／60 分鐘或永久開啟。非響鈴時短按機殼頂部中間的 BOOT 鍵可立即關屏，關屏後任一鍵喚醒。
- 鬧鐘到點會強制亮屏並響鈴；響鈴時按任一實體按鈕停止。關屏只控制顯示器，不停止時鐘、排程、網路或喇叭。Wi-Fi、方向、關屏設定、月份設定及排程均須在重啟驗收時確認。
- 非響鈴、非配網時短按「－」可切換 240×240 `檢查更新` 資訊頁，查看目前版本、已保存更新、充電 readiness 與 `/update` QR；實體鍵不會開始下載或安裝。原有任意鍵停鈴、BOOT 關屏、任意鍵喚醒及「＋」「－」十秒配網不變。
