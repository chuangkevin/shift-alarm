...
bool scheduleWorkReady=false,scheduleWorkBusy=false;uint32_t scheduleWorkId=0,scheduleGeneration=0,localSaveId=0;String localSaveState="idle",localSaveMessage,localSaveRequestId;
...
void routes() {
  ...
  const char *headers[]={"Authorization","X-Setup-Nonce","X-Save-Request"}; server.collectHeaders(headers,3);tailnetRoutes();otaStagedRoutes();localCalendarRoutes();
