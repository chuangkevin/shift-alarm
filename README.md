...
新版韌體位於 `firmware-next/`。原始碼版本為 0.3.10，後端／映像版本為 0.1.5；後端 0.1.5 與 GN100 Caddy 離線 fallback 已於 2026-09-15 部署，實機仍為 0.3.8。0.3.10 把 OTA 分成「下載並保存」與「本機驗證後安裝」，兩步都只允許 GPIO38 當下有效且 active-high 顯示充電時執行。這是刻意 fail-closed 的充電狀態，不是可靠的 USB／VBUS 偵測；電池充滿時即使接著 USB，也可能拒絕下載或安裝。0.3.8 的舊 OTA 讀取競態無法由 server chunk 策略修復，必須先用 USB 資料線 bootstrap 0.3.10；GPIO38、240×240 更新資訊頁、真實 flash marker、離線安裝與新版 live OTA 再由實機驗證。
