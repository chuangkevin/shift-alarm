...
新版韌體位於 `firmware-next/`。原始碼版本為 0.3.10，後端／映像版本為 0.1.4；尚未部署。實機仍為 0.3.8，後端仍為 0.1.3。0.3.10 把 OTA 分成「下載並保存」與「本機驗證後安裝」，兩步都只允許 GPIO38 當下有效且 active-high 顯示充電時執行。這是刻意 fail-closed 的充電狀態，不是可靠的 USB／VBUS 偵測；電池充滿時即使接著 USB，也可能拒絕下載或安裝。程式與 host 測試不能取代實機驗收；GPIO38、240×240 更新資訊頁、真實 flash marker、離線安裝與 live OTA 均待實機驗證。
