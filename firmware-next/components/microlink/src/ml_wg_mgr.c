...
static esp_err_t wg_init_interface_tcpip(microlink_t *ml) {
    ...
    if (!s_wg_output_pcb) {
        ...
        if (s_wg_output_pcb) {
            ...
            s_wg_output_pcb->local_port = ml->disco_local_port;
...
typedef struct { microlink_t *ml; SemaphoreHandle_t done; esp_err_t result; } wg_netif_call_t;
static void wg_init_call(void *arg) {
    wg_netif_call_t *call=arg;
    call->result=wg_init_interface_tcpip(call->ml);
    xSemaphoreGive(call->done);
}
static esp_err_t wg_run_netif_call(microlink_t *ml,tcpip_callback_fn fn) {
    wg_netif_call_t call={.ml=ml,.done=xSemaphoreCreateBinary(),.result=ESP_OK};
    if(!call.done)return ESP_ERR_NO_MEM;
    err_t err=tcpip_callback(fn,&call);
    if(err==ERR_OK)xSemaphoreTake(call.done,portMAX_DELAY);
    vSemaphoreDelete(call.done);
    return err==ERR_OK?call.result:ESP_FAIL;
}
static esp_err_t wg_init_interface(microlink_t *ml) {
    return wg_run_netif_call(ml,wg_init_call);
}
static void wg_shutdown_call(void *arg) {
    wg_netif_call_t *call=arg;microlink_t *ml=call->ml;
    struct netif *netif=ml->wg_netif;
    if(netif){
        wireguardif_shutdown(netif);
        netif_set_link_down(netif);netif_set_down(netif);netif_remove(netif);
        free(netif);ml->wg_netif=NULL;
    }
    if(s_wg_output_pcb){udp_remove(s_wg_output_pcb);s_wg_output_pcb=NULL;}
    xSemaphoreGive(call->done);
}
...
static int find_peer_by_id(microlink_t *ml,uint64_t id) {
    if(!id)return -1;
    for(int i=0;i<ml->peer_count;i++)if(ml->peers[i].active&&ml->peers[i].node_id==id)return i;
    return -1;
}
...
static int add_peer(microlink_t *ml, const ml_peer_update_t *update) {
    ...
    int idx = find_peer_by_id(ml, update->node_id);
    if (idx >= 0) {
    ...
    bool preserve=p->active&&p->vpn_ip==update->vpn_ip&&!memcmp(p->public_key,update->public_key,32)&&p->wg_peer_index>=0;
    if(!preserve&&p->active&&p->wg_peer_index>=0&&ml->wg_netif)wireguardif_remove_peer(ml->wg_netif,p->wg_peer_index);
    ...
    if(preserve) {
        /* Endpoint/DERP/metadata patches preserve the established crypto session. */
        p->trust_until_ms=0;p->has_direct_path=false;p->tried_initial_handshake=false;
        return idx;
    }
...
static void process_peer_updates(microlink_t *ml) {
    ...
    while (xQueueReceive(ml->peer_update_queue, &update, 0) == pdTRUE) {
        ...
        case ML_PEER_RESET:
            for(int i=0;i<ML_MAX_PEERS;i++)if(ml->peers[i].active){ml_peer_update_t gone={.node_id=ml->peers[i].node_id};remove_peer(ml,&gone);}
            memset(ml->peers,0,sizeof(ml->peers));ml->peer_count=0;
            break;
        case ML_PEER_SYNC_DONE:
            xSemaphoreTake(ml->security.lock,portMAX_DELAY);
            if(update->generation==ml->security.peer_generation&&!ml->security.peer_install_failed)ml->security.peers_ready=true;
            xSemaphoreGive(ml->security.lock);
            break;
        case ML_PEER_ADD:
            if(add_peer(ml,update)<0){
                xSemaphoreTake(ml->security.lock,portMAX_DELAY);
                if(update->generation==ml->security.peer_generation){ml->security.peer_install_failed=true;ml->security.peers_ready=false;}
                xSemaphoreGive(ml->security.lock);
            }
            break;
        case ML_PEER_REMOVE:
                    ...
                    if (!p->tried_initial_handshake) {
                         ...
                         * establish the session regardless of active flag. */
                        {
                            ...
                            if (dev && p->wg_peer_index < WIREGUARD_MAX_PEERS) {
                                { WG_CORE_GUARD; dev->peers[p->wg_peer_index].active = false; }
                            }
                        }
...
static void disco_periodic_probes(microlink_t *ml) {
    ...
    for (int n = 0; n < ml->peer_count; n++) {
        ...
        bool peer_allowed = true; /* Packet ACL is enforced after WG decryption. */
...
void ml_wg_mgr_task(void *arg) {
    ...
    int cached = 0; /* Fresh control map is authoritative; never restore stale peers. */
    if (cached > 0) {
    ...
    if (wait_bits & ML_EVT_SHUTDOWN_REQUEST) {
        ...
        xEventGroupSetBits(ml->events,ML_EVT_WG_EXIT);
    vTaskDelete(NULL);
        return;  /* Not reached */
    }
    ...
    /* This FIFO TCPIP barrier runs after prior decrypted input packets. */
    while(wg_run_netif_call(ml,wg_shutdown_call)!=ESP_OK)vTaskDelay(pdMS_TO_TICKS(100));
