...
static void make_image(unsigned char image[1024]){memset(image,0x5a,1024);esp_image_header_t header={.magic=ESP_IMAGE_HEADER_MAGIC,.chip_id=ESP_CHIP_ID_ESP32S3};esp_image_segment_header_t segment={0};esp_app_desc_t desc={.magic_word=ESP_APP_DESC_MAGIC_WORD,.version="0.3.14",.project_name="xingzhi-cube-1.54tft-wifi"};memcpy(image,&header,sizeof(header));memcpy(image+sizeof(header),&segment,sizeof(segment));memcpy(image+sizeof(header)+sizeof(segment),&desc,sizeof(desc));}
static alarm_ota_manifest_t manifest_bytes(const unsigned char *image,size_t size){alarm_ota_manifest_t m={.board="xingzhi-cube-1.54tft-wifi",.version="0.3.14",.size=(uint32_t)size};digest(image,size,m.sha256);sign_manifest(&m);return m;}
