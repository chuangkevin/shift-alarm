...
`feat/kevin-dual-alarm` 是第二顆裝置的獨立版本：Kevin 與晴晴都固定週一到週五上班，在同一頁設定兩人的個別鬧鐘，週四另有獨立時間，遠端入口為 `morning.sisihome.org`。架構、資料格式與部署隔離方式見 [0.4.x Kevin 雙人固定上班鬧鐘](docs/0.4.0-kevin-dual-alarm.md)。

新版韌體位於 `firmware-next/`。原始碼與在線裝置目前為 0.3.23；2026-09-16 已經由裝置原生 Tailscale 完成真實 OTA 下載、驗證、安裝、重開與設定保存驗收。0.3.23 補齊 TFT 的星期字型，並提高實體介面主迴圈優先權以免被 Tailnet 工作延遲。裝置 Tailnet 位址為 `100.104.66.47`，裝置到 AI／OTA 的固定入口是 GN100 `100.127.82.47:8237`；GN100 再經 Tailnet 代理到 rpi-matrix 的主要後端。
...
- 螢幕方向可保存為 0／90／180／270 度；閒置關屏可選 1／5／15／30／60 分鐘或永久開啟。非響鈴時長按機殼頂部中間的 BOOT 鍵 1.2 秒，放開後關屏；關屏後正常按下並放開中間鍵即可喚醒。
- 響鈴使用較柔和的海浪底聲與間歇鳥鳴，取代刺耳方波；最多持續 3 分鐘，未按實體按鈕時也會自動停止，避免無人在家時持續鳴叫。
